...
public class GTusachViewImpl extends Composite implements GTusachView, ClickHandler {
	...
	private void initScriptPanel() {
		User user = getUser();
		boolean isAdmin = (user != null && user.isLogon() && user.getRole().contains("admin"));
	...
	private void updateBook(int row, Book book) {
		try {
			...
			log.warning("Error updating book, row: " + row + ". " + ex.getMessage());
		}
	}
	...
	public void onPropertyChanged(PropertyChangeEvent event) {
		log.info("received event: " + event.getEventType().name());
	...
	class BookComparator implements Comparator<Book> {
		...
		public int compare(Book o1, Book o2) {
			...
			if (o2.getStatusEnum() == BookStatus.WORKING
					&& o1.getStatusEnum() != BookStatus.WORKING) {
				return 1;
			}
